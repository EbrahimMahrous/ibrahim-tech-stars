"use client";

export const webSolutionsContent = {
  title: "Web & E-Commerce Solutions",
  description:
    "Transforming ideas into exceptional digital experiences that lead to tangible results",
  heroTitle: "Building a Successful Digital Future",
  heroDescription:
    "I develop integrated web and e-commerce solutions focused on performance, conversion, and achieving business goals",

  projects: [
    {
      id: 1,
      title: "Pearline Store Dashboard",
      category: "Dashboard",
      duration: "2:45",
      youtubeId: "CCav701xyCY",
      description:
        "Developing Pearline store management interface using React and Tailwind CSS with an advanced analytics system",
      technologies: ["React", "Tailwind CSS", "Chart.js", "React Query"],
      features: [
        "Interactive Dashboard",
        "Live Charts and Statistics",
        "Order and Product Management",
        "Smart Notification System",
      ],
      challenge:
        "Client's need for an integrated monitoring and management system for his online store",
      solution:
        "Developing a custom dashboard that displays all important data in one place",
      results:
        "Improved management efficiency by 40%, reduced processing time by 60%",
      liveUrl: "#",
    },
    {
      id: 2,
      title: "Kleitech Early Kidney Failure Detection Platform",
      category: "Medical Platform",
      duration: "3:15",
      youtubeId: "iQoH2iFu3GA",
      description:
        "Specialized medical platform for early detection of kidney failure using artificial intelligence",
      technologies: ["Next.js", "TypeScript", "TensorFlow.js", "MongoDB"],
      features: [
        "Patient Data Analysis",
        "Interactive Medical Reports",
        "Medical Alert System",
        "Multilingual Interface",
      ],
      challenge:
        "Providing an accurate and easy-to-use medical diagnosis system for doctors and patients",
      solution:
        "Building an integrated platform with AI algorithms for analysis",
      results:
        "Increased diagnostic accuracy by 35%, improved patient user experience",
      liveUrl: "#",
    },
    {
      id: 3,
      title: "OTM Transportation Platform Dashboard",
      category: "Transportation",
      duration: "2:30",
      youtubeId: "KVOkDhi2uj4",
      description:
        "Integrated shipment management and transportation platform with live shipment tracking",
      technologies: ["React", "Node.js", "Socket.io", "Google Maps API"],
      features: [
        "Live Shipment Tracking",
        "Driver Management",
        "Billing System",
        "Performance Reports",
      ],
      challenge:
        "Real-time shipment tracking and logistics operations management",
      solution:
        "Developing an integrated tracking system with live maps and instant notifications",
      results:
        "Reduced tracking time by 70%, increased customer satisfaction by 45%",
      liveUrl: "#",
    },
    {
      id: 4,
      title: "Learning Management System (LMS)",
      category: "Education",
      duration: "3:45",
      youtubeId: "KP3CNoaiyBg",
      description:
        "Integrated e-learning platform with virtual classrooms and smart assessments",
      technologies: ["Next.js", "MongoDB", "WebRTC", "Stripe"],
      features: [
        "Virtual Classrooms",
        "Automatic Assessments",
        "Certified Certificates",
        "Student Tracking System",
      ],
      challenge: "Creating an interactive online learning environment",
      solution:
        "An integrated platform with live classes and smart assessment system",
      results:
        "Increased student engagement by 60%, improved learning outcomes",
      liveUrl: "#",
    },
    {
      id: 5,
      title: "Pearline E-Commerce Store",
      category: "E-Commerce",
      duration: "2:15",
      youtubeId: "jNzhuBaNC5o",
      description:
        "Integrated e-commerce store for cosmetic and personal care products",
      technologies: ["WordPress", "WooCommerce", "Elementor", "Arabic Support"],
      features: [
        "Responsive Arabic Design",
        "Multiple Payment Gateway",
        "Coupon System",
        "Order Tracking",
      ],
      challenge:
        "Creating a user-friendly e-commerce store with Arabic language support",
      solution:
        "Developing a custom store with simple user interface and smooth shopping experience",
      results: "Increased sales by 150%, improved conversion rate by 30%",
      liveUrl: "#",
    },
    {
      id: 6,
      title: "WordPress Porto Store",
      category: "E-Commerce",
      duration: "2:00",
      youtubeId: "yxFSkjhjx98",
      description:
        "Design and development of an e-commerce store using WordPress with unique designs",
      technologies: ["WordPress", "Porto Theme", "WooCommerce", "Custom CSS"],
      features: [
        "Unique Design",
        "SEO Optimization",
        "High Loading Speed",
        "Multilingual Support",
      ],
      challenge:
        "Improving the speed and performance of a WordPress store while maintaining unique design",
      solution: "Optimizing site code and adding caching technologies",
      results:
        "Improved loading speed by 65%, increased search engine visibility",
      liveUrl: "#",
    },
  ],

  skills: [
    { name: "React", iconName: "FaReact" },
    { name: "Next.js", iconName: "SiNextdotjs" },
    { name: "TypeScript", iconName: "SiTypescript" },
    { name: "Tailwind CSS", iconName: "SiTailwindcss" },
    { name: "WordPress", iconName: "FaWordpress" },
    { name: "Shopify", iconName: "FaShopify" },
    { name: "Node.js", iconName: "SiNodedotjs" },
    { name: "Express.js", iconName: "SiExpress" },
    { name: "MongoDB", iconName: "SiMongodb" },
    { name: "Performance", iconName: "FaBolt" },
    { name: "Security", iconName: "FaShieldAlt" },
    { name: "SEO", iconName: "FaSearch" },
    { name: "UX/UI Design", iconName: "FaPalette" },
    { name: "Responsive", iconName: "FaMobileAlt" },
    { name: "Analytics", iconName: "FaChartLine" },
  ],

  optimizationServices: [
    {
      id: 1,
      category: "Safety & Security",
      icon: "FaShieldAlt",
      color: "from-red-500 to-red-700",
      items: [
        "SSL Certificates and Data Encryption",
        "Firewalls",
        "Automatic Backup on Google Drive + Hostinger",
        "Protection against Cyber Attacks",
        "Continuous Monitoring of Security Vulnerabilities",
      ],
    },
    {
      id: 2,
      category: "Speed & Performance Optimization",
      icon: "FaBolt",
      color: "from-yellow-500 to-orange-500",
      items: [
        "Core Web Vitals Loading Speed Optimization",
        "Google PageSpeed Results (Mobile & Desktop)",
        "Mobile Responsiveness Improvement",
        "Mobile Cache Adjustment",
        "Improving First Response Time (FCP, LCP)",
      ],
    },
    {
      id: 3,
      category: "Technical Maintenance",
      icon: "FaCog",
      color: "from-blue-500 to-cyan-500",
      items: [
        "Cleaning Link Structure and Database",
        "Fixing Errors and Checking Broken Links",
        "Fixing 404 Errors Especially in Payment Gateways",
        "Cross-browser Compatibility",
        "Cleaning Unused Files and Data",
      ],
    },
    {
      id: 4,
      category: "Hosting & Content",
      icon: "FaServer",
      color: "from-green-500 to-emerald-500",
      items: [
        "Optimized Hosting + CDN Network",
        "CMS, Themes, and Plugins Updates",
        "Subscription and Payment Updates",
        "Automatic XML Sitemap Updates",
        "Website Hosting Management and Follow-up",
      ],
    },
    {
      id: 5,
      category: "Media Optimization",
      icon: "FaImage",
      color: "from-purple-500 to-pink-500",
      items: [
        "High-Quality Images Without Affecting Speed",
        "Fast Image Loading with Lazy Loading",
        "Image Size and Format Optimization",
        "Organized Media Library",
        "Converting Images to WebP for Faster Loading",
      ],
    },
    {
      id: 6,
      category: "Multilingual Support",
      icon: "FaGlobe",
      color: "from-indigo-500 to-blue-700",
      items: [
        "Fixing Bilingual Issues",
        "Integrated Translation",
        "SEO Optimization for Each Language",
        "Multilingual User Interface",
        "RTL Support for Arabic Language",
      ],
    },
  ],

  ctaTitle: "Ready to transform your idea into a digital reality?",
  ctaDescription:
    "Let's discuss your project and build together a digital solution that meets and exceeds your expectations",
  ctaButton1: "Schedule a Free Consultation",
  ctaButton2: "Contact Me",

  viewProject: "View Project",
  watchDemo: "Watch Demo",
  mainFeatures: "Main Features:",
  technologiesUsed: "Technologies Used:",
  viewLiveSite: "View Live Site",

  optimizationTitle: "Website Optimization and Maintenance Services",
  closeModal: "Close",
};
